...
def SignUpTenant(tenant : Tenant):
    ...
    dbcursor.execute(query, (tenant.first_name,tenant.last_name,tenant.national_insurance,tenant.email,password_hashing(tenant.password),tenant.phone_number,tenant.occupation,))
    conn.commit()
...
def LoginTenant(email : str, enteredPassword : str):
    query = "SELECT * FROM tenants WHERE email = %s;"
    conn = GetConnection()
    dbcursor = conn.cursor()    #Creating cursor object
    dbcursor.execute('USE {};'.format(prodName)) #use database'
    ...
    if tenant is None:
        ...
        print("------------------")
        return None
    hashedPassword : str = tenant[5] 
    if not bcrypt.checkpw(enteredPassword.encode('utf-8'), hashedPassword.encode('utf-8')):
        dbcursor.close()
        conn.close()
        print("Closed Database")
        print("------------------")
...
def LoginUser(email : str, enteredPassword : str):
    ...
    if not bcrypt.checkpw(enteredPassword.encode('utf-8'), hashedPassword.encode('utf-8')):
        ...
        print("------------------")
        return None
    user = User(user[0],user[1],user[2],user[3],"Blocked", user[5],user[6])
...
def UpdateTenant(tenant : Tenant):
    ...
    dbcursor.execute(query, (tenant.first_name,tenant.last_name,tenant.national_insurance,tenant.email,password_hashing(tenant.password),tenant.phone_number,tenant.occupation,tenant.references , tenant.id,))
